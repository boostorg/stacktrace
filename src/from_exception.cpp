...
inline bool PER_IS_MSVC_EH(PEXCEPTION_RECORD p) {
  DWORD EH_EXCEPTION_NUMBER = 0xE06D7363;
  ULONG_PTR EH_MAGIC_NUMBER1 = 0x19930520;
...
inline ULONG_PTR PER_PEXCEPTOBJ(PEXCEPTION_RECORD p) {
  return p->ExceptionInformation[1];
}

unsigned current_cxx_exception_index() {
...
bool is_processing_rethrow(PEXCEPTION_RECORD p) {
  // Processing flow:
  // 0. throw;
...
struct veh_installer {
  PVOID h;
  veh_installer() : h(AddVectoredExceptionHandler(1, veh)) {}
  ~veh_installer() { RemoveVectoredExceptionHandler(h); }
} installer;
